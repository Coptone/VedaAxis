...
- 提交 `36aec78` 已按所有者新口径把主结果改成单个目标的“整段机制原始总伤害 → 当前安排后的预计伤害”，AOE/死刑使用独立连续色带；计算会纳入在命中时仍有效的提前减伤。后续校准把 6 份不同击杀扩展到 P1/P2 的 59 个直接伤害行，覆盖平 A、AOE、死刑、分摊、踩塔、点名和 P2 互斥组合动作；17 个阶段/咏唱/判定行改显示为“时间轴标记”，普通平 A 只在坦克轨道计算。XIVAPI v2 Action 数据只用于复核 Action 名称和物理/魔法属性，伤害量仍取 FFLogs 匿名 P95。数据继续为 `POC_PENDING`，尚待实机命中复核。
