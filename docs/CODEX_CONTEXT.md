...
- 提交 `0a85232` 已为 AI 候选增加账号 allowlist，并在 HTTPS 环境中配置服务器侧 AI Key 和指定所有者账号。Key 只在 `/opt/vedaaxis/api/vedaaxis.env`，未进入仓库、数据库或文档；线上还有若干冒烟测试账号，但不在 AI allowlist。验证通过 API 35 项测试和 API JAR 构建；公网 JAR SHA-256 为 `372627C6C1685316CC2F207F7178B3A6003B4B856290EC1E2FB174584598CCC0`，API 内部健康检查为 `UP`，环境校验显示 AI Key 存在且 allowlist 匹配，DeepSeek `deepseek-v4-pro` 最小请求返回 HTTP 200 和 1 个 choice。备份位于 `/opt/vedaaxis/backups/20260807-1701-ai-key-0a85232`。真实计划候选生成、差异确认和排轴质量验收仍未完成。
