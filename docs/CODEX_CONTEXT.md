...
- 提交 `36aec78` 已按所有者新口径把主结果改成单个目标的“整段机制原始总伤害 → 当前安排后的预计伤害”，AOE/死刑使用独立连续色带；计算会纳入在命中时仍有效的提前减伤。FFLogs 工具可自动发现近期公开击杀，6 份不同击杀已匿名生成 11 个 P1/P2 P95 `damageProfile`，另有 4 个机制保持待校准。Web 18、API 28、Python 8、Core 28 项测试和 Dalamud Release 构建通过；该版本已部署到 HTTPS 测试环境，并在登录态页面验证 1:03“制裁之光”显示 284,938 → 197,334（30.7%，MT，红色）。数据继续为 `POC_PENDING`，尚待实机命中复核。
