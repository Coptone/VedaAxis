...
- 当前测试版是 `0.1.19`（公网 Web、API 与插件仓库已部署，API 保持 active/UP）。
- 2026-08-09 `0.1.19` 插件/API 技能绑定审计：使用本地国服 `0a0000.win32` Action 表审计 118 个规划技能，结果为 `missing=0`、`rsv=0`；计划引用的 ActionId 均存在于技能目录。插件侧补齐高等级升级/替换技能热键栏等价绑定：预警/极致防御、复仇/戮罪、暗影墙/暗影卫、星云/大星云、盾阵/圣盾阵、原初的直觉/原初的血气、石之心/刚玉之心，并保留 0.1.18 的均衡预后/均衡预后 II；明确不把神爱抚、太阳星座、油性坦培拉涂层等条件派生技能作为等价技能，避免未满足条件时误亮。API Flyway v9 修正 9 个国服显示名：武装解除、异想的祥光、坚角清汁、白牛清汁、油性坦培拉涂层、极致防御、戮罪、炽天附体、太阳星座。验证通过 Core 52 项测试、API 58 项测试、Dalamud Release 构建 0 警告 0 错误、API JAR 构建和 `git diff --check`；生产库回读 Flyway 版本为 9，9 个技能名已回读正确；公网 `pluginmaster.json` 为 `0.1.19.0`，`release/latest/VedaAxis.zip` 与 `repository/VedaAxis.zip` 均返回 HTTP 200，公网 ZIP SHA-256 为 `3c885309be70df47d7fae043dc626a6b9b3d9363a97f990532dcc5fea222d1c0`，API JAR SHA-256 为 `cee6d200b57a512315969534ebd7d91cf3b0d54d730c5cd723de7a612463ae2c`，部署备份位于 `/opt/vedaaxis/backups/20260809-0019-skill-binding-audit`。本次未更新 Web 静态资源。
- 2026-08-08 `0.1.18` 插件热修：确认 `PlanRuntime.ObserveAction` 会把一次 ActionEffect 分发给同轨所有同 ActionId 任务，导致后续 P2 同技能任务提前进入 `EARLY/LATE` 终态，实战中表现为后半段技能不再亮起。核心运行时现在只会确认已经进入亮起窗口、且仍在最晚释放/命中后 3 秒宽限内的一个最相关任务；补充 `ActionEquivalence`，让均衡预后与均衡预后 II 在确认、可用性判断和原生热键栏描边时互相等价，避免升级技能按清单 ActionId 安排但热键槽报告基础 ActionId 时不亮。验证通过 Core 40 项测试、Dalamud Release 构建 0 警告 0 错误和 `git diff --check`；公网 `pluginmaster.json` 为 `0.1.18.0`，`release/latest/VedaAxis.zip` 与 `repository/VedaAxis.zip` 均返回 HTTP 200，公网 ZIP SHA-256 为 `98abc1803358bab6a4e7d6a8166d95c9a41b35c50426008537f056721cdf8b22`，部署备份位于 `/opt/vedaaxis/backups/20260808-vedaaxis-plugin-0.1.18`。本次未更新 API JAR 或 Web 静态资源。
- 2026-08-08 `0.1.17` 插件热修：确认 `PartyListOverlay` 在原生队伍列表名字匹配失败时会按行号兜底，而 Dalamud `IPartyList` 顺序与游戏原生队伍列表行号不能可靠等价，导致白牛、混合等目标应为 MT 的单体减伤可能错误框到队伍列表第 1 行（本机玩家）。插件端已移除该行号兜底，目标框只按队友名字匹配；匹配不到时宁可不画，不再错画到其它人。验证通过 Core 34 项测试、Dalamud Release 构建 0 警告 0 错误；公网 `pluginmaster.json` 为 `0.1.17.0`，公网 ZIP SHA-256 为 `5f5fd7db32e83de68266c40dede3bff3e2a45a4fdee6d6b09c7a060c612d0268`。本次未更新 API JAR 或 Web 静态资源。
- 2026-08-08 `0.1.16` 插件热修：默认 DMU P1/P2 模板开场 45 秒内 H2 轨道本身有 8 个提醒，容易让贤者玩家误以为“开场减伤都给自己”。插件端现在在非战斗状态下按当前职业唯一匹配本机轨道；无法唯一匹配时保留手动选择并显示原因。技能槽提醒窗口和诊断任务列表会显示单体目标轨道/队友，例如“给 MT/角色名”，用于区分“我需要释放技能”和“技能目标是谁”。验证通过 Core 34 项测试、Dalamud Release 构建 0 警告 0 错误；公网 `pluginmaster.json` 为 `0.1.16.0`，`release/latest/VedaAxis.zip` 与 `repository/VedaAxis.zip` 均返回 HTTP 200，公网 ZIP SHA-256 为 `bfdc870ca2904dda92ea1629d5cf54b7018959f631ce912a1c858298bc61b13b`。本次未更新 API JAR 或 Web 静态资源，部署备份位于 `/opt/vedaaxis/backups/20260808-224742-plugin-0.1.16`。
- 2026-08-08 `0.1.15` 插件热修：线上 Nginx 日志显示插件上传请求实际命中正确路径 `POST /VedaAxis/api/v1/fight-executions`，但返回 404/401 交替；公网未登录同一路径返回 401，说明路由存在。代码排查确认 404 来自 API 业务层：待上传执行批次引用的 `planId` 不属于当前账号或已删除/旧版本，因此 `planService.get` 返回 `PLAN_NOT_FOUND`，旧插件又把它当可重试错误导致一直刷 404。插件端已改为读取上传错误体，401 仍刷新令牌重试，400/404/422 这类不可恢复旧批次会移动到本地 `failed-executions` 隔离目录并在诊断信息显示“已隔离旧批次”，不再无限重试。验证通过 Dalamud Release 构建 0 警告 0 错误、Core 34 项测试；公网插件 ZIP SHA-256 为 `707d2fac28bbd2bbc81ec5ddae9d18cb6e2362ddfbff08aa6953679ba3ef95ed`。本次未更新 API JAR 或 Web 静态资源。
- 2026-08-08 `0.1.14` 已部署：插件“计划同步”页主流程改为刷新当前账号的已发布计划列表、下拉选择具体计划并按计划 ID 精确同步；旧版 Territory/方案标签/轨道模式自动匹配保留为折叠的高级兜底。API 新增 `GET /api/v1/runtime/plans` 当前账号 ACTIVE 发布计划摘要，以及 `GET /api/v1/runtime/plans/{planId}/published` 精确发布快照。验证通过 API 全量 57 项、Core 34 项、Dalamud Release 构建 0 警告 0 错误、API JAR 构建和 `git diff --check`；公网 `pluginmaster.json` 为 `0.1.14.0`，`release/latest/VedaAxis.zip` 与 `repository/VedaAxis.zip` 均返回 HTTP 200 且 SHA-256 为 `5b6f36436bcff6c7bd7412ece0d6db32a32dc4352ae88a11e9bc7d235e4bd865`；API JAR SHA-256 为 `31fe8941ed2a227524e56882421b3a8b1bb23ac7791fd2c38f4a1ce559c5284c`，内部 `/actuator/health` 为 `UP`，公网未登录 `GET /VedaAxis/api/v1/runtime/plans` 返回 401。生产数据库只读核对显示所有者账号存在 1 个 ACTIVE 发布计划（Territory 1363、`DMU-P1P2`、`EIGHT`、v5），部署备份位于 `/opt/vedaaxis/backups/20260808-2208-runtime-plan-list`。
- 2026-08-08 `0.1.13` Web/API 追加换 T 语义已部署：Web 在平 A/死刑机制标题下提供“单体承伤目标”下拉，可选择默认当前一仇（MT/T1）、指定当前一仇为 MT/ST/T1，或标记为双坦/一二仇；计划内部兼容编码为 `当前一仇:ST` 等，不新增数据库列。Web 本地预计伤害和 API `/damage-estimates/preview` 都优先读取该机制级目标，因此换 T 后可把后续平 A/单体死刑切到 ST，最危险轨道和减伤计算会按新的当前一仇重算。验证通过 Web 42 项相关测试、`pnpm check:web`、`pnpm build:web`、API 55 项和 API JAR 构建；公网 Web 入口为 `assets/index-DKLeYi4t.js` 与 `assets/index-BUywlb4g.css`，API JAR SHA-256 为 `b82210062e87ce45bbef5e1dba457ef8a46922f53c6d99ed79a731ea671e1da4`，备份位于 `/opt/vedaaxis/backups/20260808-154939-current-enmity-target`；本次未更新 Dalamud 插件 ZIP。
- 2026-08-08 `0.1.13` Web/API 追加热修已部署：预计伤害计算使用任务自己的 `impactAtMs` 作为同机制内真实命中点，因此提前覆盖到该命中点的旧减伤会计入“已建模减伤”和“减伤后预计伤害”；跨机制提前覆盖仍按被查看机制的 `plannedAtMs` 计算。平 A/“当前一仇”类机制默认只按 MT/T1 主坦轨道计算，只有目标文本明确为“一二仇/双坦/MT-ST”等双坦目标时才同时纳入 ST，明确为“ST/副坦/二仇”时只计算副坦轨道。本次验证通过 Web 40 项、`pnpm check:web`、`pnpm build:web`、API 54 项和 API JAR 构建；公网 Web 入口为 `assets/index-LJPDRlZ1.js` 与 `assets/index-Co42ICas.css`，API JAR SHA-256 为 `5e8a8f5b72fd89279a477ca3f66c76df5114b5939a78283f631acd3cb95032c9`，备份位于 `/opt/vedaaxis/backups/20260808-153511-damage-impact-target`；本次未更新 Dalamud 插件 ZIP。
- 2026-08-08 `0.1.13` Web/API 热修已部署：任务窗口编辑器中的“机制判定”是只读参考点，不再能拖动、滑条调整或数字输入；用户只能编辑开始亮起、最早释放和最晚释放。不要把 `impactAtMs` 强制等同于 `mechanic.plannedAtMs`：现有 DMU P1/P2 默认模板中，机制开始时间和真实判定时间存在少量有意差异。AI 候选安全校验已禁止 UPDATE 现有任务时移动 `impactAtMs`；本次未更新 Dalamud 插件 ZIP。验证通过 Web 38 项、`pnpm check:web`、`pnpm build:web`、API 53 项、API JAR 构建、Core 34 项；公网 Web 入口为 `assets/index-5hlaTCsz.js` 与 `assets/index-Co42ICas.css`，API JAR SHA-256 为 `ba113718e62fa1e6c53e753bb95e5e6996790ff77a9fba04308a494b66d47c28`，备份位于 `/opt/vedaaxis/backups/20260808-151138-impact-readonly` 与 `/opt/vedaaxis/backups/20260808-151203-impact-readonly-api-retry`。
- `0.1.13` 已部署：Web/API 支持把纯治疗、纯恢复或暂未建模为直接减伤的技能安排在伤害判定后，用于“伤后抬血/恢复”；直接减伤、护盾、最大体力、无敌等仍必须覆盖判定点，发布规则会继续阻止伤后技能被误计入本次命中减伤。技能选择器在技能冷却中时，会提示上一次释放的技能、释放时间、对应机制和机制时间。插件端技能槽提醒窗口改为可开关、可锁定、可拖动，并支持 X/Y 坐标和位置预设；插件控制台改为“运行 / 计划同步 / 显示 / 诊断”页签。验证通过 Web 38 项、`pnpm check:web`、`pnpm build:web`、API 52 项、API JAR 构建、Core 34 项、Dalamud Release 构建 0 警告 0 错误；公网 Web 入口为 `assets/index-BkI2PpGk.js` 与 `assets/index-CCn5DfdV.css`，`pluginmaster.json` 为 `0.1.13.0`，公网 ZIP SHA-256 为 `41d3d8de7cc58bd42da9ffb246334eedcccb3e531ea19783e32386ea6e45a5b6`，API JAR SHA-256 为 `2d4cdcf72e7c842c29f239a42c0e3538caefe86cd2ee702fe3eaf1ec21c63fbc`，部署备份位于 `/opt/vedaaxis/backups/20260808-144726-post-impact-ui`。
- `0.1.12` 已部署：Web 计划列表支持删除计划与确认弹窗；新建计划改为“副本 → 轨道/职业 → 是否套模板”向导，并通过 `useDefaultTemplate` 区分默认模板和空白草稿；API 新增 `DELETE /plans/{planId}`，只删除当前账号计划及其版本，不删除历史复盘；DMU 四轨新增 P1/P2 时间轴兜底但不预置八轨减伤；插件计划类型支持 DMU 八轨、DMU 四轨、O8S 与自定义方案标签，运行时同步 404 会提示 Territory、方案和轨道。验证通过 Web 37 项、API 43 项、Python 9 项、Core 32 项、Dalamud Release 构建、Web/API/插件公网冒烟；公网 ZIP SHA-256 为 `cf3b8b724b436f678cfd1349aca97df50e972b6029df2938e8a8eb10aed080cb`，API JAR SHA-256 为 `4c4e6415a2153ad24dad05384f9507b50bdc46fc6f3512a0b734a7442f5b7eae`，部署备份位于 `/opt/vedaaxis/backups/20260808-0006-delete-create-sync`。
- `0.1.11` 的本地验证证据：Web 图标化技能选择器、HP 口径预计伤害条、默认隐藏无伤害时间轴标记、AI 优化上下文和插件手动停止执行上传修复均已实现；技能选择器已进一步改为居中弹窗式分类面板，快速安排栏压成单行；任务窗口改成只读摘要加可拖动/输入毫秒的时间轴弹窗，提示多轮覆盖机会/当前覆盖状态；新增同轨同技能冷却中禁选并在图标上显示剩余冷却。热修 `82f510e` 进一步修复技能目录为空或图标缺失时任务卡片文本被压入图标列的问题，并在空技能目录响应时使用本地最小目录兜底。提交 `08a78f9` 为任务窗口编辑器增加取消回滚，并允许直接拖动时间轴上的亮起、释放、最晚和判定节点。提交 `28adaa7` 把预计伤害面板内“提前覆盖到本机制”和“本地冷却预警”改为图标+文字卡片行，新增顶栏全局显示大小控制（自动、100%、112%、125%、137%，本地存储），并为个人复盘页增加刷新按钮、空响应保护和空列表排查提示；最新热修把快速安排栏右侧复选框和按钮合并为可换行动作组，并按编辑区容器宽度响应式切换 2 列/1 列，避免中间面板缩窄时裁切。验证通过 Web 35 项测试、`pnpm check:web`、`pnpm build:web`、API 32 项测试、API JAR 构建、Core 32 项测试和 Dalamud Release 构建 0 警告 0 错误。本地 API JAR SHA-256 为 `47E3AD2890518667B6171ED6E7275595CFCD24FF18CEAC587EEF9B8EF47C66B0`，本地 `pluginmaster.json` 为 `0.1.11.0`，`repository/VedaAxis.zip` SHA-256 为 `6fba6ff6b2416630f84325320c169cdabae70d35ebc437f970f34406544f7a68`。2026-08-07 通过 `ubuntu` SSH key 恢复服务器访问后，最新公网 Web 静态入口更新为 `assets/index-CDOnNKPj.js` 与 `assets/index-DZZP3J1M.css`，`pluginmaster.json`、`repository/VedaAxis.zip` 和 `release/latest/VedaAxis.zip` 均返回 HTTP 200，未登录 `POST /VedaAxis/api/v1/fight-executions` 返回 401；API 未重启，内部健康检查仍为 `UP`。服务器只读核对显示 `fight_execution` 仅有 2 条 2026-08-06 19:58–19:59（Asia/Shanghai）测试记录，后续实战记录未进入服务端表；访问日志曾出现插件上传 404/401，下一步需要在游戏端查看“执行上传状态”和本地 `pending-executions` 队列，并重连账号触发重试。
- 提交 `1a4fdd4` 已实现全局/指向 AI 优化：Web 生成候选时发送 `mode` 与 `focusTrackId`；指向优化只允许模型改目标轨道，服务端拒绝非目标轨道增删改、计划外轨道/机制/actionId、锁定项改动，以及绿色风险机制上的纯治疗/增疗资源/未建模盾刷屏。服务端会把 `attackClass`（AOE、TANK_BUSTER、AUTO_ATTACK、MECHANIC）和 `riskBasis` 传给模型，避免按裸伤害过度处理死刑。验证通过：`pnpm check:web`、Web 35 项测试、`pnpm build:web`、API 34 项测试和 API JAR 构建；公网 Web 入口为 `assets/index-BITH4aAO.js` 与 `assets/index-b2-OxdhK.css`，API JAR SHA-256 为 `51EA0454FADE9069BE121A313F4233F0FAF8E80C49D6AEAB46487825F751C9C4`。2026-08-07 16:43（Asia/Shanghai）已部署到 HTTPS 测试环境，API 内部健康检查为 `UP`，公网受保护 API 返回 401，插件仓库与 ZIP 继续 200；备份位于 `/opt/vedaaxis/backups/20260807-1643-ai-scope-1a4fdd4`。真实 AI Key/真实计划候选验收仍未完成。
- 提交 `0a85232` 已为 AI 候选增加账号 allowlist，并在 HTTPS 环境中配置服务器侧 AI Key 和指定所有者账号。Key 只在 `/opt/vedaaxis/api/vedaaxis.env`，未进入仓库、数据库或文档；线上还有若干冒烟测试账号，但不在 AI allowlist。验证通过 API 35 项测试和 API JAR 构建；公网 JAR SHA-256 为 `372627C6C1685316CC2F207F7178B3A6003B4B856290EC1E2FB174584598CCC0`，API 内部健康检查为 `UP`，环境校验显示 AI Key 存在且 allowlist 匹配，DeepSeek `deepseek-v4-pro` 最小请求返回 HTTP 200 和 1 个 choice。备份位于 `/opt/vedaaxis/backups/20260807-1701-ai-key-0a85232`。2026-08-07 17:10（Asia/Shanghai）真实计划 AI 候选请求进入 `/ai-candidates`，但公网 Nginx 返回 504，错误日志显示上游 30 秒读超时；已仅对 `/VedaAxis/api/` 调整 `proxy_read_timeout`/`proxy_send_timeout` 为 180 秒，Nginx 配置测试与 reload 成功，备份位于 `/opt/vedaaxis/backups/20260807-1724-nginx-ai-timeout`。真实计划候选生成、差异确认和排轴质量验收仍未完成。
- 2026-08-07 18:32（Asia/Shanghai）追加 API 热修：`/damage-estimates/preview` 的承伤分析改为每次预览只加载一次技能目录；AI 候选协议改为模型返回 ADD/UPDATE/DELETE 增量 operations，后端本地合成最终 assignments 并继续执行锁定项、范围、actionId、指向模式和低风险刷治疗防线校验；模型调用增加 90 秒后端读超时。验证通过 API 37 项测试和 API JAR 构建，公网 JAR SHA-256 为 `1F4D1960E6232C67BE0B6DF6F65F3E79E8B3A1E3699E677C476233E9B7B186BE`，API 内部健康检查为 `UP`，备份位于 `/opt/vedaaxis/backups/20260807-1832-ai-delta-1f4d1960`。部署后尚无新的真实 `/ai-candidates` 请求进入日志，仍需所有者重新点击 AI 优化完成候选生成和差异确认验收。
- 2026-08-07 19:23（Asia/Shanghai）AI 候选增加两项可执行安全策略：`preserveExistingAssignments` 默认开启，要求候选只能 ADD、服务端拒绝任何现有安排的 UPDATE/DELETE；`allowGcdActions` 默认关闭，服务端拒绝新增或改用 GCD 技能，但允许原计划中未变化的 GCD 保持原样。Web 将两项策略做成明确开关并随请求发送，旧客户端未发送时也采用安全默认值。Flyway v8 增加技能 `cast_category`，生产 PostgreSQL 只读回读为 118 条技能、6 条 GCD（鼓舞、士气、均衡诊断、均衡预后、魂灵风息、均衡预后 II）。验证通过 `pnpm check:web`、Web 35 项测试、`pnpm build:web`、API 40 项测试与 API JAR 构建；公网 Web 入口为 `assets/index-GaPRez9Q.js`、`assets/index-pWB-AhuL.css`，API JAR SHA-256 为 `5D3211F78D881ABDB16FD659E639839A65DA50594699B5CA4090142ED1504EDB`，Flyway 生产迁移到 v8，API 健康检查为 `UP`，备份位于 `/opt/vedaaxis/backups/20260807-192314-ai-safety`。AI 结果仍是候选，不会自动应用或发布。
