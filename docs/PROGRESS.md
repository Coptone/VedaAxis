...
VedaAxis 已形成可构建的 Web、API、Dalamud 插件和共享契约 MVP 基线，并已打通 GitHub/Gitee 代码托管与 Dalamud 自定义仓库分发。2026-08-05 的首个游戏内观察已确认匹配技能槽会高亮、超时后会变红，说明提醒状态机和覆盖层主路径已经在实机工作；该观察尚未包含版本、诊断与完整场景矩阵，因此 `POC-02` 仍保持“进行中”。当前主线仍处于“阻断 PoC + MVP 实机验收”阶段，不把单点观察或编译通过等同于完整国服实机验收。

当前优先级：

1. 关闭“开怪自动开始、脱战/团灭/完成自动结束”的战斗生命周期缺口，消除隐藏的固定 Territory ID 依赖并补齐状态诊断。
2. 先在本地完成 Web → API → PostgreSQL → 插件的在线计划与执行数据闭环，再部署一套 HTTPS 测试环境进行真实网络联调。
3. 完成 `POC-02` 原生热键栏高亮实机矩阵和 `POC-01` 国服 Dalamud 事件链验证，保存诊断、截图或录像和版本信息。
4. 收敛 `POC-04` DMU 阶段语义和绝对时间基准，使 FFLogs、插件实机与上游时间轴使用同一套锚点定义。

## 下一开发迭代：MVP-1 自动战斗与在线计划闭环

| ID | 优先级 | 工作项 | 当前基础与缺口 | 完成标准 |
| --- | --- | --- | --- | --- |
| M1-01 | P0 | 遭遇与区域绑定 | `0.1.4` 已把 Territory 加入计划契约 1.1，API/Web/C# 已同步；O8S 测试遭遇使用 Territory 755，插件显示当前/计划区域和不匹配原因 | 在 O8S 国服实机确认区域识别正确，在线计划可按 Territory 匹配，不再依赖隐藏默认值 |
| M1-02 | P0 | 自动战斗生命周期 | `0.1.4` 已加入幂等生命周期：开怪自动开始，脱战延迟、团灭/重开、完成或跨区自动结束；Core 自动测试通过 | 在 O8S 真实副本完成开怪、重复事件、脱战、团灭/重开和退出/完成闭环，并确认一把战斗只生成一次执行批次 |
| M1-03 | P0 | 本地在线数据闭环 | 账户、设备授权、计划发布/匹配、本地快照和执行上传代码已有基线 | 本地环境完成“网页注册/登录 → 创建设定并发布计划 → 插件绑定 → 拉取计划 → 实机执行 → 上传 → 网页复盘”，数据库可核对对应版本和执行记录 |
| M1-04 | P0 | HTTPS 测试环境 | 当前 `compose.yaml` 只启动 PostgreSQL/Redis，尚无 Web/API 镜像、反向代理、TLS、备份和部署脚本；API 当前未使用 Redis | 建立测试域名和 HTTPS，部署 Web、API、PostgreSQL，使用独立密钥与持久化备份，提供健康检查、日志、迁移、回滚和最小运维说明；Redis 在有实际用例前不作为首版阻断项 |
| M1-05 | P0 | 测试环境端到端验收 | 尚未经过公网/国内网络路径 | 插件仅在非战斗阶段联网并成功绑定、同步不可变快照；断网时继续使用本地快照；战斗结束异步上传，失败进入队列并在恢复后重试；网页能够查看个人执行结果 |
| M1-06 | P2 | 用户倒计时触发 | 尚未设计，且会涉及倒计时来源、重复触发、取消和队长/队员一致性 | 不进入首版；首版自动触发稳定后再设计，不能影响 `InCombat` 主触发和手动调试兜底 |

推荐实施顺序是 `M1-01 → M1-02 → M1-03 → M1-04 → M1-05`。不先把一个尚未本地联通的“完整管理端”直接搬到服务器；先证明最小数据链路，再用测试环境解决域名、HTTPS、跨网、持久化和运维问题。当前 Web 是计划、账户、设备授权和复盘入口，并不存在必须先建设的独立运营后台；运营管理功能可按真实需要另列后续范围。
...
| Stage 1：基础平台 | 进行中 | 账户、计划草稿/版本、4/8 轨编辑、规则校验、匿名分享、设备授权、计划匹配基线 | 完成本地在线数据闭环，再完成 HTTPS 测试环境部署、权限/版本边界和持久化验收 |
...
| PoC | 状态 | 证据与边界 |
| --- | --- | --- |
| POC-01 国服事件链 | 进行中 | Territory、队伍、战斗状态和 ActionEffect 接口已进入插件代码；自动启停仍受隐藏 Territory ID 限制，且缺少同一真实副本样本的完整诊断记录 |
...
- 本地在线计划与执行记录闭环通过后，HTTPS 测试环境完成同一流程，并验证断网快照与待上传队列恢复。
