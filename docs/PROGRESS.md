...
机制分类与生存分析已合并到 `main` 并部署到 HTTPS 测试环境：Web 时间轴可区分 AOE、平 A、死刑和普通机制，并显示物理/魔法属性及伤害校准状态；API 技能效果目录已覆盖当前默认 36 个技能，能够按具体目标、范围确认、持续时间、乘算和不可叠加组计算“已建模部分”的承伤结果。FFLogs 匿名伤害候选提取器已在本地样板上运行并通过测试，但单份日志无法提供跨装备/跨队伍固定入伤，因此默认 DMU 机制仍没有发布数值，不能把该能力标为真实伤害验收完成。
...
| M1-09 | P0 | 机制伤害与减伤承伤模型 | 分类 UI、36 技能效果目录、目标特定保守计算 API/UI 和 FFLogs 匿名候选提取器已合并并部署；部署构建与提交 `2316eb8` 一致，公网 Web/健康检查/新静态资源返回 200，受保护接口未登录返回预期 401。DMU 默认机制的数值、属性和多样本证据尚未发布，治疗威力护盾与无敌仍需额外输入/特判 | 每个进入公共模板的 AOE/死刑均有版本化 Action ID、伤害属性、目标模式、可追溯伤害口径和样本/公式证据；用实机角色属性回放并与实际命中交叉验证，误差和未计入项可见，不输出无证据的 100% 保证 |
