...
当前公网版本已按所有者确认的口径直接显示“整段机制对单个目标的原始总伤害 → 当前安排后的预计伤害”，并按 AOE/死刑分别着色；计算会扫描整条计划中在命中时仍有效的提前减伤，不再局限于同一时间轴行。FFLogs 工具链可自动发现近期公开击杀并匿名聚合，6 份不同击杀已为 P1/P2 生成 11 个多报告 P95 `damageProfile`，4 个证据不足的机制继续待校准。自动检查已覆盖连续色带边界、最危险轨道、跨行持续减伤、多段求和、样本去重和匿名边界；上述数值仍为 `POC_PENDING`，尚待实机命中交叉验证。
...
| M1-09 | P0 | 机制伤害与减伤承伤模型 | 已部署自动多报告采集/匿名 P95 校准、11 个默认 `damageProfile`、跨时间轴行的有效减伤计算和 AOE/死刑最终伤害色带；完整自动检查与登录态公网页面验收通过。4 个机制仍待校准，治疗威力护盾与无敌不从伤害数字中扣除 | 每个进入公共模板的 AOE/死刑均有版本化 Action ID、伤害属性、目标模式、可追溯伤害口径和样本/公式证据；用实机命中交叉验证数值，误差和未计入项可见，不输出无证据的 100% 保证 |
