...
- `0.1.6` 公网热修：修正编辑器默认阶段/机制数据晚于初始快照执行的初始化顺序，并补充兼容克隆和组件渲染回归测试；将 `refresh_token.audience` 从 32 扩至 80 字符，并增加完整设备授权集成测试。公网设备授权 API 级闭环已返回 `APPROVED`，独立游戏电脑实测仍是下一验收点。
