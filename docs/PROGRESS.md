...
HTTPS 当前已部署的上一版机制分类与生存分析可区分 AOE、平 A、死刑和普通机制，并显示物理/魔法属性及伤害校准状态；API 技能效果目录覆盖默认 36 个技能，能够按具体目标、范围确认、持续时间、乘算和不可叠加组计算“已建模部分”的承伤结果。该部署只含单份日志的匿名候选，没有把 DMU 数值放入默认计划。

当前源码进一步按所有者确认的口径改为直接显示“整段机制对单个目标的原始总伤害 → 当前安排后的预计伤害”，并按 AOE/死刑分别着色；计算会扫描整条计划中在命中时仍有效的提前减伤，不再局限于同一时间轴行。FFLogs 工具链可自动发现近期公开击杀并匿名聚合，6 份不同击杀已为 P1/P2 生成 11 个多报告 P95 `damageProfile`，4 个证据不足的机制继续待校准。自动检查已覆盖连续色带边界、最危险轨道、跨行持续减伤、多段求和、样本去重和匿名边界；上述数值仍为 `POC_PENDING`，尚待公网部署及实机命中交叉验证。
...
5. 对已生成的 11 个 P1/P2 多报告 `damageProfile` 做实机命中交叉验证，并继续补齐 4 个待校准机制；治疗属性护盾与无敌保持单独提示，不混入减伤后伤害数字。
...
| M1-09 | P0 | 机制伤害与减伤承伤模型 | 已部署上一版分类 UI 与 36 技能效果目录；当前源码新增自动多报告采集/匿名 P95 校准、11 个默认 `damageProfile`、跨时间轴行的有效减伤计算和 AOE/死刑最终伤害色带，完整自动检查通过，待公网发布。4 个机制仍待校准，治疗威力护盾与无敌不从伤害数字中扣除 | 每个进入公共模板的 AOE/死刑均有版本化 Action ID、伤害属性、目标模式、可追溯伤害口径和样本/公式证据；用实机命中交叉验证数值，误差和未计入项可见，不输出无证据的 100% 保证 |
...
- 2026-08-06：完成减伤后最终伤害预览源码与多报告校准候选。编辑器无需再输入 HP，会自动显示单目标整段原始总伤害、当前减伤率、减伤后预计伤害和最危险轨道；6 份不同公开击杀匿名聚合出 11 个 P1/P2 P95 候选，4 个机制保持待校准。Web 18 项、API 28 项、Python 8 项、Core 28 项测试以及 Dalamud Release 构建全部通过；等待公网部署和实机数值对照。
